...
--외래 키 (foreign key) 가 부모 테이블 (참조테이블)에 없다면 INSERT불가능
INSERT INTO dept2
...
UPDATE dept2
SET loca = 4000
WHERE dept_no = 10; --실패 (외래키 제약 조건 위반)

--제약 조건을 변경
--제약 조건은 추가, 삭제만 가능. 변경은 X
--변경하려면 삭제하고 새로운 내용으로 추가해야만한다.
...
CREATE TABLE members(
    ...
    M_NUM NUMBER(3) CONSTRAINT mem_memum_pk PRIMARY KEY,
    REG_DATE DATE NOT NULL CONSTRAINT mem_regdate_pk UNIQUE,
    GENDER VARCHAR2(1),
