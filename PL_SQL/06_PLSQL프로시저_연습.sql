...
BEGIN
    ...
    LOOP
        IF MOD(p_num,i) =0
        THEN v_result := v_result+1;
        END IF;
    END LOOP;
    dbms_output.put_line(p_num||'의 약수 갯수는 :'|| v_result||'개');
END;

EXEC divisor_proc(28);
/*
...
BEGIN
    ...
    ELSIF p_flag = 'D' THEN
        ...
        DELETE FROM depts
        WHERE department_id = p_department_id;
    ELSE dbms_output.put_line('해당flag에 대한 동작이 준비되지 않았습니다.');
    END IF;
...
IS
BEGIN
    MERGE INTO emps a -- 머지 할 타겟 테이블
    USING 
        (SELECT p_employee_id AS employee_id FROM dual) b
    ON 
        (a.employee_id = b.employee_id) -- 전달 받은 사번이 emps에 존재하는 지를 병합 조건으로 물어봄.
    WHEN MATCHED THEN
        UPDATE SET
    ...
    WHEN NOT MATCHED THEN
        INSERT (a.last_name, a.email,a.hire_date,a.job_id)
